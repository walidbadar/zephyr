...
#define SDHC_CDNS_DESC_MAX_LEN UINT32_MAX
 ...
 */
static void sdhc_cdns_clear_intr(const struct device *dev)
{
...
static int sdhc_cdns_setup_dma(const struct device *dev, const struct sdhc_data *data)
{
	...
	for (i = 0U; i < (table - 1U); i++) {
		sd_data->adma2_desc[i].addr = ((mem_addr_t)buff + (i * SDHC_CDNS_DESC_MAX_LEN));
		sd_data->adma2_desc[i].attr =
			SDHC_CDNS_ADMA2_DESC_TRAN | SDHC_CDNS_ADMA2_DESC_VALID;
		sd_data->adma2_desc[i].len = 0U;
	}

	sd_data->adma2_desc[table - 1U].addr = ((mem_addr_t)buff + (i * SDHC_CDNS_DESC_MAX_LEN));
	sd_data->adma2_desc[table - 1U].attr =
		SDHC_CDNS_ADMA2_DESC_TRAN | SDHC_CDNS_ADMA2_DESC_END | SDHC_CDNS_ADMA2_DESC_VALID;
	sd_data->adma2_desc[table - 1U].len = ((block_chunk) - (i * SDHC_CDNS_DESC_MAX_LEN));
 ...
 */
static uint16_t sdhc_cdns_calc_clock(uint32_t pclk, enum sdhc_clock_speed speed)
{
	uint16_t divcnt;
...
static int sdhc_cdns_set_clock(const struct device *dev, enum sdhc_clock_speed speed)
{
	...
	divisor = sdhc_cdns_calc_clock(pclk, speed);

	/* Program divisor and enable the internal clock */
	sdhc_cdns_set_bits(dev, SDHC_CDNS_CLOCK_CTRL, (uint32_t)divisor | SDHC_CDNS_CLOCK_CTRL_ICE);
...
static int sdhc_cdns_host_reset(const struct device *dev)
{
	...
	sdhc_cdns_set_bits(dev, SDHC_CDNS_HOST_CTRL, SDHC_CDNS_HOST_CTRL_DMASEL_ADMA2_64);
...
static int sdhc_cdns_card_busy(const struct device *dev)
{
	...
	ret = sdhc_cdns_wait_reg_mask(dev, SDHC_CDNS_PRESENT_STATE, 2,
				      SDHC_CDNS_PRESENT_STATE_DATSL1_MASK, 0);
	if (ret != 0) {
		return 0;
	}
...
static int sdhc_cdns_init(const struct device *dev)
{
	...
	if (sdhc_cdns_quirk_phy_init(dev)) {
		LOG_ERR("Failed to initialization phy");
		return -EIO;
	}
...
#define SDHC_CDNS_INTR_FUNC_REG(n) .irq_config_fn = sdhc_cdns_config_intr##n,

#define SDHC_CDNS_INTR_CONFIG_NULL
#define SDHC_CDNS_INTR_FUNC_REG_NULL .irq_config_fn = NULL,

#define SDHC_CDNS_INTR_CONFIG_API(n)                                                               \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, interrupts),                                          \
		(SDHC_CDNS_INTR_CONFIG(n)), (SDHC_CDNS_INTR_CONFIG_NULL))
