...
static int co5300_mipi_dsi_cmd_write(const struct device *dev, uint8_t cmd,
				  uint8_t *data, size_t length)
{
	const struct co5300_config *config = dev->config;

	return mipi_dsi_dcs_write(config->mipi_dev, config->channel,
				cmd, data, length);
}

static int co5300_mipi_dsi_display_transfer(const struct device *dev,
		...
		const uint8_t *tx_buf, uint32_t tx_len)
{
	...
	while (tx_len > 0) {
		...
		if (local_desc->pitch > local_desc->width) {
			...
			tx_buf += line_each_sent * local_desc->pitch * data->bytes_per_pixel;
			tx_buf += bytes_written % (local_desc->width * data->bytes_per_pixel);
		} else {
...
static int co5300_mspi_write_helper(const struct device *dev, uint8_t opcode,
				    uint8_t cmd, const uint8_t *data, size_t length)
{
	const struct co5300_config *config = dev->config;

	uint32_t header = ((uint32_t)opcode << 24) | ((uint32_t)cmd << 8);
	...
	return spi_write(config->spi_dev, &config->mspi_config, &buf_set);
}

static int co5300_mspi_cmd_write(const struct device *dev, uint8_t cmd,
				  uint8_t *data, size_t length)
{
	return co5300_mspi_write_helper(dev, CO5300_MSPI_OPCODE_WRITE_CMD, cmd, data, length);
}

static int co5300_mspi_display_transfer(const struct device *dev,
		struct display_buffer_descriptor *local_desc,
		const uint8_t *tx_buf, uint32_t tx_len)
{
	struct co5300_data *data = dev->data;
	...
	for (i = 0; i < local_desc->height; i++) {
		uint8_t ram_cmd = (i == 0) ? MIPI_DCS_WRITE_MEMORY_START
					     : MIPI_DCS_WRITE_MEMORY_CONTINUE;

		ret = co5300_mspi_write_helper(dev, CO5300_QSPI_OPCODE_WRITE_COLOR,
					ram_cmd, tx_buf, tx_len);
		if (ret < 0) {
			return ret;
		}

		tx_buf += local_desc->pitch * data->bytes_per_pixel;
	}
		...
		const void *buf)
{
	...
	ret = config->cmd_write(dev, MIPI_DCS_SET_COLUMN_ADDRESS, cmd_params,
				sizeof(cmd_params));
	if (ret < 0) {
		return ret;
	}
	...
	ret = config->cmd_write(dev, MIPI_DCS_SET_PAGE_ADDRESS, cmd_params,
				sizeof(cmd_params));
	if (ret < 0) {
		return ret;
	}
	...
	tx_buf = data->frame_ptr + (local_y * data->frame_pitch * data->bytes_per_pixel)
		+ (local_x * data->bytes_per_pixel);
	tx_len = local_desc.buf_size;

	ret = config->display_transfer(dev, &local_desc, tx_buf, tx_len);
	if (ret < 0) {
		return ret;
	}
...
static int co5300_set_brightness(const struct device *dev, const uint8_t contrast)
{
	...
	return config->cmd_write(dev, MIPI_DCS_SET_DISPLAY_BRIGHTNESS, (uint8_t *)&contrast, 1);
}
		...
		const enum display_pixel_format pixel_format)
{
	 ...
	 */
	ret = config->cmd_write(dev, MIPI_DCS_SET_ADDRESS_MODE, &cmd_params[0], 1U);
	if (ret < 0) {
		return ret;
	}

	ret = config->cmd_write(dev, MIPI_DCS_SET_PIXEL_FORMAT, &cmd_params[1], 1U);
	if (ret < 0) {
		return ret;
	}
...
static int co5300_init(const struct device *dev)
{
	...
	while (ptr_to_cmd_register < ptr_to_last_cmd) {
		...
		ret = config->cmd_write(dev, cmd_register, &cmd_params, cmd_param_size);
		if (ret < 0) {
			return ret;
		}
	}
	...
	/* Set pixel format */
	if (data->pixel_format == CO5300_PIXFMT_RGB888) {
		co5300_set_pixel_format(dev, PIXEL_FORMAT_RGB_888);
	} else if (data->pixel_format == CO5300_PIXFMT_RGB565) {
	...
	ret = config->cmd_write(dev, MIPI_DCS_EXIT_SLEEP_MODE, NULL, 0);
	if (ret < 0) {
		return ret;
	}
...
#define CO5300_CONFIG_MSPI(node_id)                                                      \
	{                                                                                    \
		...
		.cmd_write = co5300_mspi_cmd_write,                                              \
		.display_transfer = co5300_mspi_display_transfer,                                \
		.mspi_config = SPI_CONFIG_DT_INST(node_id,                                       \
						  SPI_OP_MODE_MASTER | SPI_WORD_SET(8) | SPI_LINES_QUAD, 0),     \
...
#define CO5300_CONFIG_MIPI_DSI(node_id)                                                  \
	{                                                                                    \
		.mipi_dev = DEVICE_DT_GET(DT_INST_BUS(node_id)),                                 \
		.cmd_write = co5300_mipi_dsi_cmd_write,                                          \
		.display_transfer = co5300_mipi_dsi_display_transfer,                            \
...
#define CO5300_DEVICE_INIT(node_id)								\
	static const struct co5300_config co5300_config_##node_id =				\
		COND_CODE_1(DT_INST_ON_BUS(node_id, spi), (CO5300_CONFIG_MSPI(node_id)),  \
